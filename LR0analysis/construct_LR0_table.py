...
class LR:
    def __init__(self):
        ...
        self.Vt = ['a', 'b', 'c', 'd']
        self.start = 'E'  # 开始符号
        self.production = ['E->aA', 'E->bB', 'A->cA', 'A->d', 'B->cB', 'B->d']  # 产生式
        self.project = []  # 项目
    ...
    def covert_lr0to_json(self):
        ...
        for act in self.action.keys():
            action[str(act)] = {}
            for key in self.action[act]:
                if self.action[act][key][0] == 'S':
                    action[str(act)][key] = str(self.action[act][key].split('S')[1])
                elif self.action[act][key] == 'acc':
                    action[str(act)][key] = 'acc'
                elif self.action[act][key][0] == 'r':
                    r_index = int(self.action[act][key].split('r')[1])
                    action[str(act)][key] = self.production[r_index - 1].split('->')[0] + self.production[r_index - 1].split('->')[1]

        for go in self.goto.keys():
            goto[str(go)] = {}
            for key in self.goto[go]:
                if str(self.goto[go][key]).isdigit():
                    goto[str(go)][key] = str(self.goto[go][key])
        # print(action)
        # print(goto)
        dict = {'action': action, 'goto': goto}

        filepath = './Table/LR0_2.json'
